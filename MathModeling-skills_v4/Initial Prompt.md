...
- Default to `interaction_mode: learning` and `rigor_profile: submission`; do not drop to `lean` unless the user explicitly asks. The full G1–G7 pipeline (planning → method card → risk probe → decision → code → run summary → final explanation → solution package → robustness → frozen numbers → paper → G6 audits → G7 review) must be completed before delivery.
- Do not skip G1–G7 stages or jump directly to writing the paper. Every gate's artifacts must exist on disk before the next stage begins. If artifacts are missing, go back and produce them — do not write paper sections based on assumptions.
