...
3.5. **Mixed-model extension assessment (mandatory).**
   - Inspect the data profile for: repeated measures (same subject multiple times), hierarchical/grouped structure, clustering, panel/longitudinal data, or substantial individual heterogeneity.
   - If any such structure exists, the `main_candidate` MUST be evaluated against a mixed/hierarchical alternative:
     - Continuous response → Linear Mixed Model (LMM) or hierarchical Bayesian model;
     - Binary/categorical response → Generalized Linear Mixed Model (GLMM) or GEE;
     - Count/survival response → appropriate mixed-effects or frailty model.
   - Record the extension assessment in the method card: `mixed_extension: {applicable: true/false, proposed: "LMM/GLMM/GEE/...", reason: "...", adopted: true/false}`.
   - If a mixed model is adopted as main, the original simple model becomes the baseline. If not adopted, the reason must be substantive (not "too complex" or "not needed").
   - This step is mandatory per Initial Prompt Rule 5. Never skip it.
