...
2.5. **Mixed-model implementation check (mandatory).**
   - If the method card's `mixed_extension.adopted` is true, the code plan MUST include:
     - the grouping variable (e.g., subject ID, cluster ID);
     - the random-effects structure (random intercept, random slope, or both);
     - the estimation method (REML/ML for LMM, PQL/Laplace for GLMM, GEE with covariance structure);
     - a comparison metric against the simple baseline (e.g., likelihood ratio test, AIC/BIC, ICC).
   - If `mixed_extension.adopted` is false but the data has repeated measures or grouping, record a warning in the code plan explaining why the simple model's standard errors may be underestimated.
   - Set all random seeds to `454520` (see Initial Prompt default).
