...
→ contest-paper-reviewer  (G7 强制关卡：≥90分才允许交付，否则返回修改后重审)
→ workflow-orchestrator
...
- QA 没通过前，不要组装最终论文。
- 竞赛评审（G7 — CONTEST_REVIEW_PASSED）未达到90分前，不要交付最终论文。G7 是最终交付的强制关卡，任何情况下不得跳过；低于90分必须根据评审意见修改后重新评审。
...
- 规则4：论文必须经过 contest-paper-reviewer 评审且总分≥90分才能交付，低于90分必须根据评审意见修改后重新评审。
