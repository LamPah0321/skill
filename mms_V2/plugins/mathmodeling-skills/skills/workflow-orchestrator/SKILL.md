...
- the latest review total score is **≥ 90/100**;
...
If the score is < 90, route to `paper-polisher` / `paper-section-writer` for targeted fixes per the review's "关键提升建议", then re-run `contest-paper-reviewer`. Do not deliver, do not set `final_delivery=true`.
...
- G6 passed but G7 (contest review) not passed → `contest-paper-reviewer`; if score < 90, route fixes to `paper-polisher` / `paper-section-writer` then re-review. **Never deliver before G7 ≥ 90.**
...
- **require G7 (contest-paper-reviewer ≥ 90) before any final delivery**;
...
- **G7 enforcement: `final_delivery` is false until contest review score ≥ 90; no delivery occurs before G7 passes.**
