...
→ contest-paper-reviewer  (G7 强制关卡：≥95分才允许交付，否则返回修改后重审)
→ workflow-orchestrator
...
- QA 没通过前，不要组装最终论文。
- 竞赛评审（G7 — CONTEST_REVIEW_PASSED）未达到95分前，不要交付最终论文。G7 是最终交付的强制关卡，任何情况下不得跳过；低于95分必须根据评审意见修改后重新评审。
- 不要修改 workspace/data_raw/ 下的原始数据。
- 不要编造数据、数值结果、参考文献、图表、实验或模型性能结论。
- 所有 Python 代码中的随机种子（random_state, np.random.seed, torch.manual_seed 等）默认使用 `454520`，除非有明确理由使用其他值。
...
- 规则4：论文必须经过 contest-paper-reviewer 评审且总分≥95分才能交付，低于95分必须根据评审意见修改后重新评审。
- 规则5（混合模型扩展）：主方法选定后，必须评估是否可扩展为混合/层次/贝叶斯模型以提升创新性。若数据存在重复测量、层次结构、聚类结构或个体异质性，必须优先考虑混合效应模型（LMM/GLMM/GEE）或分层贝叶斯模型，并在方法卡中记录"扩展评估"结论（采用/不采用及理由）。禁止仅用最简单的OLS/Logistic而不评估更复杂的混合模型。
- 规则6（深化结果讨论）：每个子问题的结果分析必须包含四层讨论：(1)数字结果的实际意义与物理/业务解释；(2)与基线方法或文献结论的对比及差异原因；(3)反直觉或异常结果的深入分析（不得回避）；(4)结果的局限性与适用边界。禁止只报数字不做解释，禁止用"结果表明"一笔带过。
