...
The submission report MUST include a "敏感性分析与结果讨论" section that:
- identifies the 2–3 most load-bearing parameters and perturbs them over a meaningful range (not just ±5%);
- reports how the headline result changes (direction, magnitude, threshold crossings);
- explains WHY the result is sensitive or robust to each parameter (mechanistic explanation, not just "it changed");
- connects sensitivity findings back to the result discussion's "局限性与适用边界" layer.
A robustness report that only lists PASS/FAIL without parameter perturbation and mechanistic explanation is INCOMPLETE.
